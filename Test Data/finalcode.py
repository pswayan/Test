...
def upload_file_api():
    ...
    # Handle file upload
        ...
        logging.error('Access forbidden. Invalid API key or user not in the IT-QE team')
...
def get_tsn():
    ...
    if user and user.role == 'IT-QE'and api_key == secret_key:
        ...
        default_path = os.path.join('TestData','tsn_list.csv')
...
# if __name__ == '__main__':
#     # Enable HTTPS by specifying the certificate and private key
#     app.run(debug=True, ssl_context=('path_to_certificate.crt', 'path_to_private_key.key'))